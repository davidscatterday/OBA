...
def get_normalized_score(sector):
    ...
    if not scores.empty:
        return scores.iloc[0]['Normalized_Score_2']
    else:
        return None
...
def get_normalized_score2(sector):
    ...
    if not scores.empty:
        return scores.iloc[0]['normal_score_graph1']
    else:
        return None
...
def optimize_portfolio_allocation(df, max_harm_score):
    ...
    # Update portfolio allocation based on optimized weights using .loc[]
...
if submit_button:
    try:
        ...
        if hist.empty:
            ...
            # Update portfolio allocation using .loc[]
